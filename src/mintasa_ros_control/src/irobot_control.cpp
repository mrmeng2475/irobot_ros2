...
class MintasaWholeBodyController : public rclcpp::Node
{
public:
    MintasaWholeBodyController()
    : Node("mintasa_whole_body_controller")
    {
        // ================= 1. 配置反向关节 =================
        r_inverted_joint_ids_ = {1, 3, 5, 6};         // 右臂反向关节
        l_inverted_joint_ids_ = {11, 13, 14, 15, 16}; // 左臂反向关节
        // 头部反向逻辑直接在回调中处理（通过取负号）
        ...
        RCLCPP_INFO(this->get_logger(), "头部关节规划速度: %.2f", head_velocity);
        ...
        // 检查所有 18 个电机是否全部在线 (右臂1-8, 左臂11-18, 头部21-22)
        auto found_ids_vec = pController->getActuatorIdArray();
        std::vector<uint8_t> required_ids = {1, 2, 3, 4, 5, 6, 7, 8, 11, 12, 13, 14, 15, 16, 17, 18, 21, 22};
        ...
        RCLCPP_INFO(this->get_logger(), "✅ 已成功找到全身所有执行器 (ID 1-8, 11-18, 21-22)。");
        ...
        r_joint_sub_ = this->create_subscription<sensor_msgs::msg::JointState>(
            "/r_joint_cmd", 10, std::bind(&MintasaWholeBodyController::rJointCommandCallback, this, _1));
        r_clip_sub_ = this->create_subscription<irobot_interfaces::msg::ClipCommand>(
            "/r_clip_cmd", 10, std::bind(&MintasaWholeBodyController::rClipCommandCallback, this, _1));
        ...
        l_joint_sub_ = this->create_subscription<sensor_msgs::msg::JointState>(
            "/l_joint_cmd", 10, std::bind(&MintasaWholeBodyController::lJointCommandCallback, this, _1));
        l_clip_sub_ = this->create_subscription<irobot_interfaces::msg::ClipCommand>(
            "/l_clip_cmd", 10, std::bind(&MintasaWholeBodyController::lClipCommandCallback, this, _1));
        l_state_pub_ = this->create_publisher<sensor_msgs::msg::JointState>("/hardware/l_joint_states", 10);
        
        // --- 头部订阅与发布 ---
        head_sub_ = this->create_subscription<irobot_interfaces::msg::HeadCommand>(
            "/head_cmd", 10, std::bind(&MintasaWholeBodyController::headCommandCallback, this, _1));
        head_state_pub_ = this->create_publisher<sensor_msgs::msg::JointState>("/hardware/head_joint_states", 10);

        // ================= 5. 统一状态反馈定时器 =================
        timer_ = this->create_wall_timer(
            std::chrono::milliseconds(20), 
            std::bind(&MintasaWholeBodyController::feedbackTimerCallback, this));
            
        RCLCPP_INFO(this->get_logger(), "🚀 全身体统一控制器启动完毕，监听各自的指令流！");
    }
    ...
    ~MintasaWholeBodyController()
    {
        ...
        {
            RCLCPP_INFO(this->get_logger(), "正在安全禁用全身所有执行器...");
            for(const auto& id : actuator_ids_) {
...
private:
    ...
    void feedbackTimerCallback()
    {
        ...
        auto head_feedback_msg = sensor_msgs::msg::JointState();
        
        auto now = this->get_clock()->now();
        
        r_feedback_msg.header.stamp = now;
        l_feedback_msg.header.stamp = now;
        head_feedback_msg.header.stamp = now;
        ...
        for (const auto& id : actuator_ids_)
        {
            ...
            if (id == 8 || id == 18) {
                ...
                // 映射回 head_joint1 和 head_joint2
                head_feedback_msg.name.push_back("head_joint" + std::to_string(id - 20)); 
                head_feedback_msg.position.push_back(final_position);
            } 
            else {
                // 机械臂处理 (转换单位并考虑正负号)
                final_position = actuator_units_to_radians(current_actuator_pos);
        ...
        head_state_pub_->publish(head_feedback_msg);
    }
    ...
    // 头部资源
    rclcpp::Subscription<irobot_interfaces::msg::HeadCommand>::SharedPtr head_sub_;
    rclcpp::Publisher<sensor_msgs::msg::JointState>::SharedPtr head_state_pub_;
...
int main(int argc, char **argv)
{
    ...
    rclcpp::spin(std::make_shared<MintasaWholeBodyController>());
    rclcpp::shutdown();
    return 0;
}